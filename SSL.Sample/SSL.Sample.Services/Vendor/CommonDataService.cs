...
namespace SSL.Sample.SSL.Sample.Services.Vendor
{
    public class CommonDataService
    {
        ...
        DataSet _ds;
        DataTable _dt;
        private readonly string _connectionString = @"Data Source=192.168.15.100,1419\\MSSQLSERVER2019;Initial Catalog=SSLSupportDB_DevV2;User ID=sa;password=S123456_;TrustServerCertificate=true; connect Timeout=1000";
        private readonly string _connectionStringVat = "";
        ...
        public List<T> ExecuteStoredProcedure<T>(string procedureName, string callName, string desc, params object[] parameters)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                using (var command = new SqlCommand(procedureName, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@CallType", callName);
                    command.Parameters.AddWithValue("@Desc", desc); // Add the @Desc parameter

                    // Add other parameters dynamically based on the provided array
                    for (int i = 0; i < parameters.Length; i++)
                    {
                        command.Parameters.AddWithValue($"@Desc{i + 1}", parameters[i]);
                    }
