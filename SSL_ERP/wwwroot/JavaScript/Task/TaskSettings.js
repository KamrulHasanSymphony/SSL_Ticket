...
var TaskSummaryManager = {
                ...
                parameterMap: function (options) {
                    if (options.filter && options.filter.filters) {
                        options.filter.filters.forEach(function (filter) {
                            ...
                            if (filter.field === "clientsName") {
                                filter.value = "tc.ClientsName";
                            }
                        });
                    }
                    return options;
    ...
    SaveHoldTime: function (selectedId) {
        debugger;
        var objHold = TaskSummaryHelper.CreateHoldTimeObject(selectedId);
        debugger;
        $.ajax({
            url: '/Task/SaveStopTime',
            method: 'post',
    ...
    IsStackHolder: function (selectedId, callback) {
        $.ajax({
            url: '/Task/GetStackHolderById',
            method: 'get',
            data: { selectedId: selectedId },
            success: function (response) {
                
                callback(response); // Convert the response to boolean if needed
            },
            error: function (xhr, status, error) {
...
var TaskSummaryHelper = {
    ...
    GenerateTaskGrid: function () {
        $("#kTaskGrid").kendoGrid({
            ...
            columns: [
                ...
                //        return "<a class='btn btn-primary btn-sm mr-2 edit' title='Edit Task' href=/Task/Edit/" + dataItem.taskId + ">" + "<i class='fas fa-pencil-alt'></i>" + "</a>"
                //            +
                //            "<a class='btn btn-success btn-sm mr-2' title='Go to Ticket' href='/Ticket/Edit/" + dataItem.t_TicketId + "'>" +
                ...
                //},
                {
                    ...
                    width: 150,
                    template: function (dataItem) {
                        return "<a class='btn btn-primary btn-sm mr-2 edit' title='Edit Task' href='/Task/Edit?Id="
                            + encodeURIComponent(dataItem.taskId)
                            + "&ticketTitle=" + encodeURIComponent(dataItem.ticketTitle)
                            + "&ticketCode=" + encodeURIComponent(dataItem.ticketCode) + "'>"
                            + "<i class='fas fa-pencil-alt'></i>"
                            + "</a>"
                            +
                            "<a class='btn btn-success btn-sm mr-2' title='Go to Ticket' href='/Ticket/Edit?id="
                            + encodeURIComponent(dataItem.t_TicketId) + "'>"
                            + "<i class='fas fa-th-list'></i>"
                            + "</a>";
                    }
                },
                ...
                {
                    field: "ticketCode", title: "Ticket Code", sortable: true, width: 150
                },
                {
                    field: "taskCode", title: "Task Code", sortable: true, width: 150
                },
                {
                    field: "taskTitle", title: "Title", sortable: true, width: 220
                },
                {
                    field: "taskDescription", title: "Description", sortable: true, hidden: true, width: 220
                },
                {
                ...
                {
                    field: "prioritiesName", title: "Priority", sortable: true, width: 150
                },
                {
                    field: "statusName", title: "Status", sortable: true, width: 150
                },
                ...
                {
                    field: "workingTime", title: "Working Time", sortable: true, width: 150
                },
                {
            ...
            columns: [
                {
                    ...
                    template: function (dataItem) {
                        var pauseButton = "";
                        var completeButton = "";
                        var holdButton = ""; // Declare the hold button

                        // Check the startStatus to determine if the buttons should be enabled or disabled
                        if (dataItem.startStatus !== 'C' && dataItem.startStatus !== 'P' && dataItem.startStatus !== 'H') {
                            ...
                            holdButton = "<a style='background-color: orange;' href='#' class='btn btn-success btn-sm mr-2 edit ' title='Hold' onclick='TaskSummaryHelper.holdTask(" + JSON.stringify(dataItem) + ")'><i class='fa fa-pause'></i> </a>"; // Hold button
                        } else {
                            ...
                            holdButton = "<a style='background-color: orange; cursor: not-allowed;' href='#' class='btn btn-success btn-sm mr-2 edit disabled' title='Hold' onclick='return false;'><i class='fa fa-pause'></i> </a>"; // Disabled Hold button
                        }

                        // Return the buttons (Pause, Complete, Hold)
                        return pauseButton + completeButton + holdButton;
                    }
                },
    ...
    GenerateKendoComboBox: function () {
        ...
        //var taskType = new kendo.data.DataSource({
        //    transport: {
        //        read: {
        //            url: "/Task/GetAllTaskTypeData",
        //            dataType: "json"
        //        }
        //    }
        //});
    ...
    ClickEvents: function () {
        ...
        $("#btnSaveEnternal").click(function () {
            ...
            if (taskId == 0 || taskId == null) {
                ...
                toastr.warning("Create Task first!");
        ...
        $("#btnHold").click(function (event) {
            event.preventDefault();
            TaskSummaryManager.SaveHoldTime(selectedId);
        });
        $("#btnTimeStart").click(function (e) {
            e.preventDefault();
            var ticketId = $("#T_TicketId").val();
            TaskSummaryManager.IsStackHolder(ticketId, function (data) {
                ...
                if (data === true) {
                    ...
                    toastr.warning("You are not Stack Holder");
                    //alert("You are not Stack Holder");
                    $("#divTimeModal").data("kendoWindow").close();
                }
            });
        });
    ...
    CreateHoldTimeObject: function (selectedId) {
        debugger;
        var obj = new Object();
        obj.id = selectedId.id;
        obj.t_TaskId = $("#Id").val();
