var BranchProfileController = function ( BranchProfileService) {
    ...
    $('.btnActivate').click('click', function () {
        Confirmation("Are you sure? Do You Want to Activate Branch?", function (result) {
            console.log(result);
            if (result) {
                ...
                var form = $("#frm_Branch")[0];
                var formData = new FormData(form);
                BranchProfileService.activateBranch(formData, activateDoneBranch, activateFail);
    ...
    function activateDoneBranch(result) {
        if (result.status == "200") {
            ShowNotification(1, "Branch Activate Successfully");
        }
        else if (result.status == "400") {
